...
- set `DATABASE_URL`
- run Prisma migrate deploy for the sqlite-first local schema history
- run `npm run hosted:db:reconcile` from `apps/web` to align hosted Postgres with the deployed vendor auth / vendor commercial contract
...
Persistence truth:
- `NEXT_PUBLIC_APP_URL` and `NOTIFY_TRANSPORT` should read back through `vercel env run -e production`
- sensitive values such as `SMTP_URL` and `INTERNAL_AUTOMATION_SECRET` should be treated as runtime-only; verify them with a fresh production deploy plus a live automation probe, not by expecting local plaintext readback
...
7. Run `npm run test:hosted` or the GitHub hosted regression workflow and keep the green receipt
