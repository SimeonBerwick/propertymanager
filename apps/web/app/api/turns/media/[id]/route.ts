...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const task = await prisma.unitTurnTask.findFirst({ where: { id, turn: { orgId: session.userId } }, select: { photoUrl: true } })
  if (!task?.photoUrl) return new Response('Not found', { status: 404 })
  const media = await readStoredMedia(task.photoUrl); if (!media) return new Response('Not found', { status: 404 })
