...
export default async function NewPropertyPage({ searchParams }: { searchParams: Promise<{ propertyType?: string }> }) {
  const session = await getLandlordSession()
  if (!session) redirect('/login?error=session-expired')
  const propertyType = (await searchParams).propertyType === 'cooperative' ? 'cooperative' : 'single_family'
    ...
    <div className="stack" style={{ maxWidth: 640, margin: '0 auto' }}>
      ...
      <section className="card stack">
        ...
        <p className="muted" style={{ margin: 0 }}>
          {propertyType === 'cooperative' ? 'Co-op Mode is ready. Add the cooperative building and its initial units to begin board approvals and recurring building work.' : 'Choose the property type. Apartment communities can create their initial units all at once.'}
        </p>
      ...
      <section className="card stack">
        <NewPropertyForm initialPropertyType={propertyType} />
      </section>
    </div>
  )
}
