...
export async function respondToBoardApprovalAction(formData: FormData) {
  ...
  if (['returned', 'declined'].includes(response) && !note) redirect(`/board/${token}?error=${encodeURIComponent('Add a note so the property manager knows what needs to change.')}` as Route)
