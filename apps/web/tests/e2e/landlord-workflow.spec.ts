...
test('landlord can complete the core maintenance workflow in the browser', async ({ page }) => {
  ...
  const commentForm = messagesDisclosure.locator('form').filter({ has: page.getByLabel('Who should see this?') })
  await expect(commentForm).toBeVisible()
  await commentForm.getByLabel('Who should see this?').selectOption('internal')
  await expect(commentForm.getByRole('button', { name: 'Save internal note' })).toBeVisible()
