...
export function getRequestNextAction(request: NextActionRequest, now = new Date()): RequestNextAction {
  ...
  if ((request.vendorPayableBalanceCents ?? 0) > 0) {
    return { ...base, id: `${request.id}:payment`, primaryLabel: 'Mark vendor paid', reason: `Amount owed to ${request.vendorPayableTo ?? 'vendor'} is still open.`, group: 'Payments to finish', priority: request.status === 'closed' ? 'high' : 'normal', actionType: 'mark_vendor_paid', score: request.status === 'closed' ? 88 : 72 }
  }
  ...
  if (request.reviewState === 'vendor_completed_pending_review' || request.status === 'completed') {
    return { ...base, id: `${request.id}:completion-review`, primaryLabel: 'Review completion', reason: 'The vendor says work is complete and needs manager review.', group: 'Completion review', priority: 'high', actionType: 'review_completion', score: 84 }
  }
  ...
  if (!request.assignedVendorName && ['approved', 'vendor_selected', 'reopened'].includes(request.status)) {
    return { ...base, id: `${request.id}:assign`, primaryLabel: 'Assign vendor', reason: 'The work is approved but no vendor is assigned.', group: 'Vendor assignment', priority: 'normal', actionType: 'assign_vendor', score: 58 }
  }
