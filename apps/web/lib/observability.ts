...
async function getRequestLogContext(): Promise<RequestLogContext> {
  try {
...
export async function logAppEvent(level: LogLevel, event: string, details: LogDetails = {}) {
  const context = await getRequestLogContext()
...
export async function logServerActionError(action: string, error: unknown, details: LogDetails = {}) {
  await logAppError(`server_action.${action}.failed`, error, {
    action,
    ...details,
  })
}
